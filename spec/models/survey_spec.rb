...
describe Survey do
  context '.generate' do
    ...
    context 'related questions' do
      it 'returns related question after parent question' do
        user = create(:user)
        first_question = create(:question, absolute_index: 0)
        related_question = create(:question)
        first_question.related_questions << related_question
        20.times.map { create(:question) }
        survey = Survey.generate(user)
        survey.survey_questions.first.question.id.should == first_question.id
        survey.survey_questions.second.question.id.should == related_question.id
